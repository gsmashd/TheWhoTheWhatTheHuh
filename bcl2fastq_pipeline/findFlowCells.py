...
def rerunFlowcell(config):
    seq_data_path = af.SEQUENCER_OUTPUTFOLDER[config.get("Options","runID").split("_")[-2]]
    ss, opts = getSampleSheets(os.path.join(config.get("Paths","baseDir"),seq_data_paht,config.get("Options","runID")))
